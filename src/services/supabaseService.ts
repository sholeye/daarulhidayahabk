...
export const updateStudentDB = async (
  ...
  if (updates.amountPaid !== undefined)
    dbUpdates.amount_paid = updates.amountPaid;
  if (updates.totalFee !== undefined) dbUpdates.total_fee = updates.totalFee;
  if (updates.guardian) {
    if (updates.guardian.name !== undefined)
      dbUpdates.guardian_name = updates.guardian.name;
    if (updates.guardian.phone !== undefined)
      dbUpdates.guardian_phone = updates.guardian.phone;
    if (updates.guardian.occupation !== undefined)
      dbUpdates.guardian_occupation = updates.guardian.occupation;
    if (updates.guardian.stateOfOrigin !== undefined)
      dbUpdates.guardian_state = updates.guardian.stateOfOrigin;
  }
...
export const fetchAcademicTerms = async (): Promise<AcademicTerm[]> => {
  const { data, error } = await supabase
    .from("academic_terms")
    .select("*")
    ...
    .order("created_at", { ascending: false });
  if (error) throw error;
  return (data || []).map(mapAcademicTermFromDB);
};

export const createAcademicTerm = async (
  term: Partial<AcademicTerm>,
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      fee: term.fee,
      is_current: term.isCurrent ?? false,
    ...
    .single();
  if (error) throw error;
  return mapAcademicTermFromDB(data);
};

export const updateAcademicTermDB = async (
  id: string,
  ...
  if (updates.startsOn !== undefined)
    dbUpdates.starts_on = updates.startsOn || null;
  if (updates.endsOn !== undefined) dbUpdates.ends_on = updates.endsOn || null;
  const { data, error } = await supabase
    .from("academic_terms")
    .update(dbUpdates)
    .eq("id", id)
...
export const deleteAcademicTermDB = async (id: string): Promise<void> => {
...
export const bulkUpsertAttendance = async (
  ...
  const { error } = await supabase
    .from("attendance")
    .upsert(dbRecords, { onConflict: "student_id,date" });
  if (error) throw error;
};
...
export const updatePaymentDB = async (
  ...
  if (payment.amount !== undefined) updates.amount = Number(payment.amount);
  if (payment.paymentMethod !== undefined)
    updates.payment_method = payment.paymentMethod;
  if (payment.date !== undefined) updates.date = payment.date;
  if (payment.termId !== undefined) updates.term_id = payment.termId;
  if (payment.term !== undefined) updates.term = payment.term;
  if (payment.session !== undefined) updates.session = payment.session;
...
export const deletePaymentDB = async (id: string): Promise<void> => {
...
export const updateAnnouncementDB = async (
  id: string,
...
export const updateBlogPostDB = async (
  id: string,
...
export const toggleBlogLikeDB = async (
  postId: string,
  userId: string,
): Promise<boolean> => {
...
export const fetchClassSubjects = async (): Promise<ClassSubject[]> => {
  const { data, error } = await supabase
    .from("class_subjects")
    .select("*")
    .order("name");
...
export const createSchoolClass = async (schoolClass: {
  name: string;
  nameArabic: string;
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      name_arabic: schoolClass.nameArabic,
      level: schoolClass.level,
    })
    .select()
    .single();
...
export const updateSchoolClassDB = async (
  id: string,
  updates: Partial<SchoolClass>,
): Promise<void> => {
  const dbUpdates: Record<string, unknown> = {};
  if (updates.name !== undefined) dbUpdates.name = updates.name;
  if (updates.nameArabic !== undefined)
    dbUpdates.name_arabic = updates.nameArabic;
...
export const deleteSchoolClassDB = async (id: string): Promise<void> => {
...
export const deleteClassSubjectDB = async (id: string): Promise<void> => {
...
function mapAcademicTermFromDB(row: Record<string, unknown>): AcademicTerm {
  return {
    id: row.id as string,
    name: row.name as string,
    session: row.session as string,
    fee: normalizeCurrency(Number(row.fee) || 0),
