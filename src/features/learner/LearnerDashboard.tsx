...
export const LearnerDashboard: React.FC = () => {
  ...
  const studentResult = currentStudent
    ? results.find((r) => r.studentId === currentStudent.studentId)
    : null;

  const studentAttendance = currentStudent
    ? attendance.filter((a) => a.studentId === currentStudent.studentId)
    : [];
  ...
  const activeAnnouncements = announcements
    .filter((a) => a.isActive)
    .slice(0, 3);
  const currentAmountPaid = currentStudent
    ? getTermAmountPaid(payments, currentStudent.studentId, currentTerm)
    : 0;
  const currentFee = currentTerm?.fee || 0;
  const currentFeeStatus = getFeeStatus(currentAmountPaid, currentFee);
  const feesPaid = currentFeeStatus === "paid";
  const feeBalance = getOutstandingBalance(currentFee, currentAmountPaid);
      ...
      >
        ...
        <div className="bg-card rounded-2xl border border-border p-12 text-center">
          ...
          <p className="text-muted-foreground">
            No student record found for your account. Please contact the
            administrator.
          </p>
        </div>
      </motion.div>
        ...
        {[
          {
            ...
            sub: (
              <>
                <span className="text-primary">{presentDays} present</span>{" "}
                <span className="text-secondary">{lateDays} late</span>
              </>
            ),
            bg: "bg-primary/10",
            color: "text-primary",
          ...
          },
          {
            icon: FiDollarSign,
            label: "Fee Status",
            value: currentTerm ? currentFeeStatus : "Not set",
            sub: currentTerm
              ? feeBalance > 0
                ? `Balance: ${formatCurrency(feeBalance)}`
                : "Fully paid"
              : "No current term",
            bg: feesPaid ? "bg-primary/10" : "bg-destructive/10",
            color: feesPaid ? "text-primary" : "text-destructive",
            badge: true,
          },
          {
            icon: FiUser,
          ...
          >
            ...
            {item.badge ? (
              <Badge
                variant={
                  feesPaid
                    ? "paid"
                    : currentStudent.feeStatus === "partial"
                      ? "partial"
                      : "unpaid"
                }
              >
            ...
            <p className="text-xs text-muted-foreground mt-2 flex gap-2 truncate">
              {item.sub}
            </p>
          </motion.div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-card rounded-2xl border border-border p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-foreground">Academic Result</h2>
            {studentResult && (
              <Badge variant={feesPaid ? "paid" : "unpaid"}>
                {feesPaid ? "Unlocked" : "Locked"}
              </Badge>
            )}
          </div>
          {studentResult ? (
            <>
              <div className="flex items-center gap-4 mb-4">
                <div className="w-16 h-16 rounded-full bg-gradient-to-br from-primary to-secondary flex items-center justify-center">
                  <span className="text-2xl font-bold text-primary-foreground">
                    {studentResult.averageScore.toFixed(0)}%
                  </span>
                </div>
                <div>
                  <p className="font-medium text-foreground">
                    {studentResult.term} - {studentResult.session}
                  </p>
              ...
              {feesPaid ? (
                ...
                <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-4 text-center">
                  <FiXCircle className="w-8 h-8 mx-auto text-destructive mb-2" />
                  <p className="text-sm text-destructive font-medium">
                    Result Locked - Fees Not Paid
                  </p>
                </div>
              )}
            </>
            ...
            <div className="bg-muted/50 rounded-lg p-6 text-center">
              <FiFileText className="w-10 h-10 mx-auto text-muted-foreground mb-2" />
      ...
      <div className="bg-card rounded-2xl border border-border p-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center p-4 rounded-lg bg-primary/10">
            <FiCheckCircle className="w-6 h-6 mx-auto text-primary mb-2" />
            <p className="text-2xl font-bold text-foreground">{presentDays}</p>
            <p className="text-xs text-muted-foreground">Days Present</p>
          </div>
          <div className="text-center p-4 rounded-lg bg-secondary/10">
            <FiClock className="w-6 h-6 mx-auto text-secondary mb-2" />
          ...
          <div className="text-center p-4 rounded-lg bg-destructive/10">
            <FiXCircle className="w-6 h-6 mx-auto text-destructive mb-2" />
