...
const RegistrationModal: React.FC<RegistrationModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!currentTerm) {
      toast.error(
        "Create and select a current academic term before registering students.",
      );
      return;
    }
    onSubmit(
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, guardianState: e.target.value })
                  }
                  placeholder="Ogun State"
                />
              </div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    feeStatus: e.target.value as "paid" | "partial" | "unpaid",
                  })
                }
                className="w-full h-10 px-3 rounded-lg border border-input bg-background text-foreground"
              >
...
const StudentDetailModal: React.FC<{
  ...
  const { academicTerms, currentTerm, payments } = useSharedData();
  const [qrCode, setQrCode] = useState<string>("");
  const [copied, setCopied] = useState(false);
  const [selectedTermId, setSelectedTermId] = useState("");

  React.useEffect(() => {
    setSelectedTermId(currentTerm?.id || academicTerms[0]?.id || "");
  }, [student?.id, currentTerm?.id, academicTerms]);
  React.useEffect(() => {
    if (student)
  ...
  const amountPaidForTerm = getTermAmountPaid(
    payments,
    student.studentId,
    selectedTerm,
  );
  const feeForTerm = selectedTerm?.fee || 0;
  const balanceForTerm = getOutstandingBalance(feeForTerm, amountPaidForTerm);
  const feeStatusForTerm = getFeeStatus(amountPaidForTerm, feeForTerm);
  const credentials = generateStudentCredentials(
    student.fullName,
    student.studentId,
  );
        ...
        <div className="p-6 space-y-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <Badge
                variant={
                  feeStatusForTerm === "paid"
                    ? "paid"
                    : feeStatusForTerm === "partial"
                      ? "partial"
                      : "unpaid"
                }
                className="mt-2"
              >
                {selectedTerm ? feeStatusForTerm : "No term configured"}
              </Badge>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div className="col-span-2 p-4 rounded-xl bg-primary/5 border border-primary/10">
            ...
            <div>
              ...
              <p className="font-medium text-foreground">
                {formatCurrency(amountPaidForTerm)}
              </p>
            </div>
            <div>
              <p className="text-muted-foreground">Term Fee</p>
              <p className="font-medium text-foreground">
                {selectedTerm ? formatCurrency(feeForTerm) : "Not set"}
              </p>
            </div>
            <div>
              <p className="text-muted-foreground">Balance</p>
              <p className="font-medium text-destructive">
                {formatCurrency(balanceForTerm)}
              </p>
            </div>
          </div>
          {qrCode && (
...
const CredentialsModal: React.FC<{
  ...
  studentName: string;
}> = ({ isOpen, onClose, credentials, studentName }) => {
  const [copied, setCopied] = useState(false);
      ...
      <div className="bg-card rounded-2xl border border-border shadow-strong w-full max-w-md animate-scale-in">
        ...
        <div className="p-6 space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            Share these credentials with the student. Keep the password safe.
          </p>
  ...
  const handleRegister = async (
    ...
    try {
      ...
      if (!authResult.success || !authResult.userId) {
        throw new Error(
          authResult.message || "Failed to create student login account",
        );
      }
