    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          {
            icon: FiDollarSign,
            value: `${myChildren.filter((c) => getFeeStatus(getTermAmountPaid(payments, c.studentId, currentTerm), currentTerm?.fee || 0) === "paid").length}/${myChildren.length}`,
            label: "Fees Paid",
            bg: "bg-primary/10",
            color: "text-primary",
          },
          {
            icon: FiFileText,
            value: myChildren.filter((c) =>
              results.some((r) => r.studentId === c.studentId),
            ).length,
        ...
        <div className="space-y-4">
          {myChildren.length === 0 ? (
            <div className="text-center py-8">
              <FiUsers className="w-10 h-10 mx-auto text-muted-foreground mb-3" />
              <p className="text-muted-foreground">
                No children linked to your account. Please contact the
                administrator.
              </p>
            </div>
          ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <div className="text-center p-2 rounded-lg bg-background">
                      <p className="text-lg font-bold text-foreground">
                        {result ? `${result.averageScore.toFixed(0)}%` : "N/A"}
                      </p>
            ...
            activeAnnouncements.map((ann) => (
              <div key={ann.id} className="p-4 rounded-xl bg-muted/50">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Badge
                    variant={
                      ann.category === "urgent" ? "destructive" : "outline"
                    }
                  >
                    {ann.category}
                  </Badge>
