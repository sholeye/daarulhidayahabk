...
import type { AcademicTerm, Payment, Student } from "@/types";
...
export const getOutstandingBalance = (
  ...
  return Math.max(
    0,
    normalizeCurrency(totalFee) - normalizeCurrency(amountPaid),
  );
};
...
export const getPaymentProgress = (
  ...
  return Math.min(
    100,
    Math.round((normalizeCurrency(amountPaid) / safeTotalFee) * 100),
  );
};
...
export const getTermAmountPaid = (
  payments: Payment[],
  studentId: string,
  term?: AcademicTerm,
): number => {
    ...
    payments
      .filter(
        (payment) =>
          ...
          (payment.termId === term.id ||
            (!payment.termId &&
              payment.term === term.name &&
              payment.session === term.session)),
      )
...
export const isResultAccessible = (
  student?: Pick<Student, "amountPaid" | "totalFee"> | null,
): boolean => {
